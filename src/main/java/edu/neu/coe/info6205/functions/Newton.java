 ...
 * It is an example of a non-deterministic algorithm inasmuch as the convergence
 * (or lack thereof) is very dependent
 * on the value of the initial guess x0 to the solve method.
 * However, if you run it with identical starting conditions, it will always
 * come out the same: it does not use any random elements.
 */
public class Newton {
     ...
     *                  If this is too far from any root, the solution may not
     *                  converge.
    ...
    public static void main(String[] args) {
        ...
        // Newton's Approximation problem: e^x - 3x = 0
        Newton newton1 = new Newton("e^x - 3x", x -> Math.exp(x) - 3 * x, x -> Math.exp(x) - 3);

        // Solve the problem starting with a value of x = 1;
        // requiring a precision of 10^-7;
        // and giving up after 200 tries.
        Either<String, Double> result1 = newton.solve(1.0, 200, 1E-7);
